#!/bin/sh
# FastAgent Installer
# Usage: curl -fsSL https://raw.githubusercontent.com/tokenaissance/fastagent/main/install.sh | sh
# Or:    FASTAGENT_INSTALL_DIR=~/bin curl -fsSL ... | sh
set -e

REPO="tokenaissance/fastagent"
BINARY="fastagent"
...
choose_install_dir() {
  if [ -n "${FASTAGENT_INSTALL_DIR:-}" ]; then
    INSTALL_DIR="$FASTAGENT_INSTALL_DIR"
  else
...
main() {
  ...
  printf "\n"
  success "FastAgent ${VERSION} installed → ${INSTALL_DIR}/${BINARY}"
  printf "\n"

  if [ "${NEEDS_SOURCE:-0}" = "1" ]; then
    printf "  ${YELLOW}Run this to activate:${NC}\n"
    printf "    source %s\n\n" "$SHELL_RC"
    printf "  Or open a new terminal, then run: ${BOLD}fastagent${NC}\n"
  else
    printf "  Run: ${BOLD}fastagent${NC}\n"
